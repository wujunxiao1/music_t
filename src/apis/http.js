...
// 发送验证码
const sentCode = ({ phone = '', ctcode = '+86' }) => { return api.get(`/captcha/sent?phone=${phone}&ctcode=${ctcode.substring(1)}`, {})};
// 验证验证码
const verifyCode = ({ phone = '', ctcode = '+86', captcha = '' }) => { return api.get(`/captcha/verify?phone=${phone}&ctcode=${ctcode.substring(1)}&captcha=${captcha}`, {})};
// 手机号快捷登录
const loginPhone = ({ phone = '', password = '', ctcode = '+86', captcha = '' }) => { return api.post(`/login/cellphone`, {phone, password, countrycode: ctcode.substring(1), captcha }) };
// 退出登录
